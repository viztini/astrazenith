#!/usr/bin/env python3
"""
AstraZenith — Python coding agent CLI with multi-model support.

Usage:
  python astra_zenith.py [options] [prompt]
...
def cmd_mcp(args: str, _state, _config) -> bool:
    ...
    if not servers:
        ...
        if not configs:
            ...
            info("Add servers in ~/.astra_zenith/mcp.json or .mcp.json")
    ...
    if total_tools:
        info(f"Total: {total_tools} MCP tool(s) available to Astra")
    return True
...
def cmd_voice(args: str, state, config) -> bool:
    ...
    if subcmd == "status":
        ...
        info("  Env override: ASTRAZENITH_WHISPER_MODEL (default: base)")
        return True
...
def main():
    parser = argparse.ArgumentParser(
        prog="astra_zenith",
        description="AstraZenith — multi-provider coding agent CLI",
        add_help=False,
    )
