...
def _message_char_count(m: dict) -> int:
    """Count text-bearing characters in one neutral-format message."""
    total = 0
...
def estimate_tokens(messages: list) -> int:
    ...
    """
    return int(sum(_message_char_count(m) for m in messages) / 3.5)
...
def find_split_point(messages: list, keep_ratio: float = 0.3) -> int:
    ...
    if not messages:
        return 0
    # One char pass; total matches estimate_tokens (int(sum/3.5)), running matches per-msg int division
    chars = [_message_char_count(m) for m in messages]
    total = int(sum(chars) / 3.5)
    if total <= 0:
        return 0
    per_msg = [int(c / 3.5) for c in chars]
