...
This document is for developers who want to understand, modify, or extend AstraZenith.
...
AstraZenith is a Python CLI that lets LLMs (GPT, Gemini, Anthropic, etc.) operate as
...
User Input
    │
    ▼
astra_zenith.py  ── REPL, slash commands, rendering
    │
...
File-based memory system stored in `~/.astra_zenith/memory/`.
...
**Search order:** Project-level (`./.astra_zenith/skills/`) overrides user-level
(`~/.astra_zenith/skills/`) when skill names collide.
...
Defaults stored in `~/.astra_zenith/config.json`. Key settings:
...
```
1. astra_zenith.py captures input
...
11. astra_zenith.py renders diff with ANSI colors (red/green)
