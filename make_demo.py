#!/usr/bin/env python3
"""
Generate animated GIF demo of AstraZenith using PIL.
...
def build_scenes():
    ...
    # ── Scene 2: Assistant response header appears ───────────────────────
    pre = BANNER + [prompt_line(msg1)]
    add(pre + [None, astra_header(), [seg("│ ", SUBTEXT)]], 300)

    # ── Scene 3: Tool call - Glob ────────────────────────────────────────
    base = pre + [None, astra_header()]
    add(base + [
        tool_line("⚙", "Glob", "**/*.py"),
    ], 500)
    add(base + [
    ...
    # ── Scene 5: Assistant streams response ──────────────────────────────
    response_lines = [
        ...
        " 553  astra_zenith.py — REPL entry point, slash commands, rich rendering",
        "────────────────────────────────────────────────────",
        "1261  total",
        "",
        "The largest file is `astra_zenith.py` containing the interactive REPL,",
    ...
    add(base + tool_section + [text_line(l, 2) for l in response_lines] + [astra_sep()], 1200)

    # ── Scene 6: New prompt appears ──────────────────────────────────────
    full1 = (pre + [None, astra_header()] +
             tool_section +
    ...
    msg2 = "Write a hello_world.py that prints 'Hello from AstraZenith!'"
    ...
    base2 = full1 + [prompt_line(msg2), None, astra_header()]
    add(base2 + [
    ...
    add(base2 + [
        ...
        None,
        tool_line("⚙", "Bash", "python3 /tmp/hello_world.py"),
        tool_ok("→ Hello from AstraZenith!"),
    ], 800)
    ...
    resp2 = [
        ...
        "",
        "  print('Hello from AstraZenith!')",
        "",
        "Output: Hello from AstraZenith!",
    ]
    tool2 = [
        tool_line("⚙", "Write", "/tmp/hello_world.py", MAUVE),
        tool_ok("Wrote 3 lines to /tmp/hello_world.py"),
    ...
    add(base2 + tool2 + [text_line(l, 2) for l in resp2] + [astra_sep()], 1500)

    # ── Scene 10: Slash command demo ─────────────────────────────────────
    final_state = (full1 + [prompt_line(msg2), None, astra_header()] +
                   tool2 + [text_line(l, 2) for l in resp2] + [astra_sep(), None])
...
def render_screenshot(output_path="screenshot.png"):
    ...
    lines = (
        ...
        [
            ...
            text_line(" 553  astra_zenith.py — REPL + slash commands", 2),
            text_line("────────────────────────────────", 2),
            text_line("1261  total", 2),
            None,
            text_line("The main entry point `astra_zenith.py` contains the REPL,", 2),
