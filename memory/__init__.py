"""Memory package for AstraZenith.

Provides persistent, file-based memory across conversations.

Storage layout:
  user scope    : ~/.astra_zenith/memory/<slug>.md   (shared across projects)
  project scope : .astra_zenith/memory/<slug>.md     (local to cwd)

The MEMORY.md index in each directory is auto-maintained and injected
into the system prompt so Astra has an overview of available memories.
