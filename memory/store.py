...
Storage layout:
  user scope    : ~/.astra_zenith/memory/<slug>.md
  project scope : .astra_zenith/memory/<slug>.md  (relative to cwd)

MEMORY.md in each directory is the index file — rebuilt automatically after
every save/delete. It is loaded into the system prompt to give Astra an
...
USER_MEMORY_DIR = Path.home() / ".astra_zenith" / "memory"
...
def get_project_memory_dir() -> Path:
    ...
    return Path.cwd() / ".astra_zenith" / "memory"
...
def get_memory_dir(scope: str = "user") -> Path:
    ...
    Args:
        scope: "user" (global ~/.astra_zenith/memory) or
               "project" (.astra_zenith/memory relative to cwd)
    """
    if scope == "project":
