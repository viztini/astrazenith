...
def load_agent_definitions() -> Dict[str, AgentDefinition]:
    ...
    Search paths:
      ~/.astra_zenith/agents/*.md   (user-level)
      .astra_zenith/agents/*.md     (project-level, overrides user)
    """
    defs: Dict[str, AgentDefinition] = dict(_BUILTIN_AGENTS)

    # User-level
    user_dir = Path.home() / ".astra_zenith" / "agents"
    if user_dir.is_dir():
    ...
    proj_dir = Path.cwd() / ".astra_zenith" / "agents"
    if proj_dir.is_dir():
...
def _create_worktree(base_dir: str) -> tuple:
    ...
    """
    branch = f"astrazenith-agent-{uuid.uuid4().hex[:8]}"
    # mkdtemp gives us a path; remove the empty dir so git can create it
    wt_path = tempfile.mkdtemp(prefix="astrazenith-agent-wt-")
    os.rmdir(wt_path)
    subprocess.run(
