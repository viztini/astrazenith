...
class TestSplitIdentifier:
    def test_camel_case(self):
        ...
        assert split_identifier("AstraZenith") == ["Astra", "Zenith"]
...
class TestReplVoiceIntegration:
    def test_voice_in_commands(self):
        import astra_zenith
        assert "voice" in astra_zenith.COMMANDS

    def test_voice_command_callable(self):
        import astra_zenith
        assert callable(astra_zenith.COMMANDS["voice"])
    ...
    def test_handle_slash_voice_sentinel(self):
        ...
        with patch.object(astra_zenith, "cmd_voice", return_value=sentinel):
            # Re-bind in COMMANDS so the patch is seen
            astra_zenith.COMMANDS["voice"] = astra_zenith.cmd_voice
            result = astra_zenith.handle_slash("/voice", object(), {})
    ...
    def test_voice_status_no_crash(self, capsys):
        ...
        # Should not raise
        try:
            astra_zenith.cmd_voice("status", object(), {})
        except SystemExit:
            pass
    ...
    def test_voice_lang_set(self, capsys):
        import astra_zenith
        astra_zenith.cmd_voice("lang zh", object(), {})
        assert astra_zenith._voice_language == "zh"
        # Reset
        astra_zenith.cmd_voice("lang auto", object(), {})
        assert astra_zenith._voice_language == "auto"
