"""Tool definitions and implementations for AstraZenith."""
import json
import os
import re
...
# ── Tool JSON schemas (sent to the provider / LLM API) ─────────────────────
...
TOOL_SCHEMAS = [
    ...
    # ── Task tools (schemas also listed here for the model's tool list) ────────
    {
        "name": "TaskCreate",
        "description": (
...
def _webfetch(url: str, prompt: str = None) -> str:
    try:
        import httpx
        r = httpx.get(url, headers={"User-Agent": "AstraZenith/1.0"},
...
def _ask_user_question(
    ...
    The REPL loop (astra_zenith.py) periodically calls drain_pending_questions()
